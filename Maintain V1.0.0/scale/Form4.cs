...
namespace scale
{
    public partial class Form4 : Form
    {
        ...
        {
            string q = @"SELECT [القسم] ,count(*) 'العدد' FROM [MYDB].[dbo].[TicketView] "
                     + " where [تاريخ العطل] >= '" + dateTimePicker1.Text + "' and [تاريخ العطل]<= '" + dateTimePicker2.Text + "'"
                      +" group by [القسم]";
            SqlDataAdapter adp;
        ...
        private void WorkerBTN_Click(object sender, EventArgs e)
        {
            ...
            string q = @"SELECT [نوع العامل] as 'اسم العامل',count(*) 'العدد' FROM [MYDB].[dbo].[TicketView] "
        ...
        {
            string q = @"SELECT [نوع الماكينة/ المرفق] as 'الماكينة' ,count(*) 'العدد' FROM [MYDB].[dbo].[TicketView] "
                   + " where [تاريخ العطل] >= '" + dateTimePicker1.Text + "' and [تاريخ العطل]<= '" + dateTimePicker2.Text + "'"
        ...
        {
            string q = @"SELECT [نوع العطل] as 'اسم العطل' ,count(*) 'العدد' FROM [MYDB].[dbo].[TicketView] "
                   + " where [تاريخ العطل] >= '" + dateTimePicker1.Text + "' and [تاريخ العطل]<= '" + dateTimePicker2.Text + "'"
                    + " group by [نوع العطل] order by العدد desc";
            SqlDataAdapter adp;
            adp = new SqlDataAdapter(q, sqlcon);
            DataTable dt = new DataTable();
            try
        ...
        {
            string q = @"SELECT [نوع الماكينة/ المرفق] as 'الماكينة' ,sum(datediff(minute,[وقت التوقف],[وقت التشغيل])) as 'فترة التوقف' FROM [MYDB].[dbo].[TicketView] "
                   + " where [تاريخ العطل] >= '" + dateTimePicker1.Text + "' and [تاريخ العطل]<= '" + dateTimePicker2.Text + "'"
        ...
        private void button6_Click(object sender, EventArgs e)
        {
            ...
            string q = @"SELECT [القسم] as 'الماكينة' ,sum(datediff(minute,[وقت التوقف],[وقت التشغيل])) as 'فترة التوقف' FROM [MYDB].[dbo].[TicketView] "
                   + " where [تاريخ العطل] >= '" + dateTimePicker1.Text + "' and [تاريخ العطل]<= '" + dateTimePicker2.Text + "'"
                   + " group by [القسم] ";
            SqlDataAdapter adp;
            adp = new SqlDataAdapter(q, sqlcon);
